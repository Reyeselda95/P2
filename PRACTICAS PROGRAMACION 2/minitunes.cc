...
SongBin songToBinary(const Song &song){
  SongBin songbin;
  songbin.id=song.id;
  strncpy(songbin.title, song.title.c_str(), kTITLE);
  songbin.title[kTITLE]='\0';
  strncpy(songbin.artist, song.artist.c_str(),kARTIST);
  songbin.artist[kARTIST]='\0';
  strncpy(songbin.album, song.album.c_str(),kALBUM);
  songbin.album[kALBUM]='\0';
  strncpy(songbin.genre, song.genre.c_str(),kGENRE);
  songbin.genre[kGENRE]='\0';
  strncpy(songbin.url, song.url.c_str(),kURL);
  songbin.url[kURL]='\0';
  return(songbin);
}
